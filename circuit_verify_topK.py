"""
E. Causal verification of attribution-top nodes on GB01-B.

For each candidate (L, #n) drawn from the GB01 attribution top list, run B forward
with one intervention on (L, #n), then measure
    M = sum_{anchor positions} act[L33, #9054]
under each of:
    baseline   no intervention
    ablate     act[..., n]  = 0          (silence the neuron)
    amplify    act[..., n] *= 2          (double the signal)
    invert     act[..., n] *= -1         (flip the signal)

Effect size:  (M_intervened - M_baseline) / |M_baseline|.

A node whose ablation moves M by >5% (relative) on the SAME B prompt is a
real causal driver of L33#9054, not just an attribution gradient artifact.
"""
...
# Candidates from CIRCUIT_FINDINGS_2026-05-01.txt attribution top:
#   (layer, neuron, attribution_diff, label)
CANDIDATES = [
    (12, 4638,  -44.0, "headliner mid-early"),
    (24, 5330,  +17.0, "L24 hub candidate"),
    ( 9, 6039,  +16.0, "early"),
    (28, 5445,  -14.0, "late"),
    (13, 1565,   -9.0, "mid"),
]
...
def main():
    ...
    torch.save(out, OUT_DIR / "circuit_verify_topK.pt")
    print(f"\n[save] {OUT_DIR / 'circuit_verify_topK.pt'}")
