"""
Dual-segment region-aware QA differential (code domain, think mode).

9 regions:
  0  unlabeled (prompt + special tokens + outside-of-labels)
  1  T_glue   2  T_d1   3  T_d2   4  T_d3     (synthetic thought side)
  5  A_glue   6  A_d1   7  A_d2   8  A_d3     (gemini answer side)

Pipeline:
  1. Generate Gemma's full thought+answer in think mode (cached after first run)
  2. Build reference B = prompt + <|channel> + 'thought\\n' + synth + '\\n'
                               + <channel|> + gemini_answer + <turn|>
  3. Detect <|channel> / <channel|> / <turn|> positions in both A and B
  4. Build labels:
       B labels: difflib-mapped from synth_text (T_*) + gemini_text (A_*)
       A labels: copy B's labels by positional alignment within each segment
                 (truncated to min thought len + min answer len)
  5. Forward both, hook accumulates per-(layer, region, neuron) stats
  6. Diff B-A per region, print top neurons per region
"""
...
GEMINI_PATH = ROOT / "data" / "gemini_3_flash_preview_code_qa_01.json"
OUT_DIR = ROOT / "outputs" / "code_region_qa01"
...
T_BASE = 1  # T_glue=1, T_d1=2, T_d2=3, T_d3=4
A_BASE = 5  # A_glue=5, A_d1=6, A_d2=7, A_d3=8


# ---------- text -> token region labels (region_probe.py style) ----------
...
def label_segment_tokens(segment_text, offsets, conclusion, glue, base):
    n = len(offsets)
...
def build_reference_and_labels(tokenizer, syn_data, gem_data):
    """Returns: ref_ids[N], ref_labels[N], thought_lo, thought_hi, answer_lo, answer_hi (all in ref_ids frame)."""
    ...
    syn_text = syn_data["synthetic_thought"]
    gem_text = gem_data["output"]

    syn_enc = tokenizer(syn_text, add_special_tokens=False, return_offsets_mapping=True)
    syn_ids = syn_enc["input_ids"]
    syn_offsets = syn_enc["offset_mapping"]
    syn_labels, syn_found, syn_missed = label_segment_tokens(
        syn_text, syn_offsets,
        syn_data.get("conclusion_analysis", {}),
        syn_data.get("glue_sentences", []),
        base=T_BASE,
    )
    print(f"  [label B-thought] found={syn_found}, missed={syn_missed}")

    gem_enc = tokenizer(gem_text, add_special_tokens=False, return_offsets_mapping=True)
    gem_ids = gem_enc["input_ids"]
    gem_offsets = gem_enc["offset_mapping"]
    gem_labels, gem_found, gem_missed = label_segment_tokens(
        gem_text, gem_offsets,
        gem_data.get("conclusion_analysis", {}),
        gem_data.get("glue_sentences", []),
        base=A_BASE,
    )
    print(f"  [label B-answer] found={gem_found}, missed={gem_missed}")
    ...
    for tid, lab in zip(syn_ids, syn_labels.tolist()):
    ...
    for tid, lab in zip(gem_ids, gem_labels.tolist()):
        full_ids.append(tid); full_labels.append(lab)
...
def find_a_segments(gen_ids, soc_id, eoc_id, eot_id):
    """Find thought_lo/hi and answer_lo/hi (content boundaries) within gen_ids."""
    ...
    # thought content begins right after <|channel>; intro 'thought\n' tokens are part of unlabeled
...
# ---------- forward + hook (per-region stats) ----------
def forward_capture_regions(model, layers, full_ids, span_start, region_labels):
    T = full_ids.shape[0]
    span_len = T - span_start
    assert region_labels.shape[0] == span_len, f"labels {region_labels.shape[0]} vs span {span_len}"
...
# ---------- reporting ----------
def render_top(diff, stats_a, stats_b, n_layers, region_id, label, top=20, l_lo=15):
...
def main():
    ...
    gem_data = json.loads(GEMINI_PATH.read_text(encoding="utf-8"))
    user_input = syn_data["input"]
    ...
    if a_seg is None:
        print("[err] could not find segments in Gemma generation")
        return
    a_th_lo, a_th_hi, a_an_lo, a_an_hi = a_seg
    print(f"[seg-A] thought=[{a_th_lo},{a_th_hi}) ({a_th_hi-a_th_lo} tokens), answer=[{a_an_lo},{a_an_hi}) ({a_an_hi-a_an_lo} tokens)")
    ...
    ref_ids, ref_labels, b_th_lo, b_th_hi, b_an_lo, b_an_hi = build_reference_and_labels(
        tokenizer, syn_data, gem_data,
    )
    print(f"[seg-B] thought=[{b_th_lo},{b_th_hi}) ({b_th_hi-b_th_lo} tokens), answer=[{b_an_lo},{b_an_hi}) ({b_an_hi-b_an_lo} tokens)")
    ...
    # ---- shadow labels for A: copy from B at aligned positions ----
    ...
    # ---- truncate B labels to aligned regions only ----
    ...
    # ---- print top per region ----
    for r in range(1, NUM_REGIONS):
        if counts_a[r] >= 6 and counts_b[r] >= 6:  # need enough samples in BOTH
